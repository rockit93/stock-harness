...
def _frame_payload(frame, limit: int = 500) -> list[dict[str, Any]]:
    ...
    timestamps = output[first_column]
    has_intraday_time = (timestamps.dt.normalize() != timestamps).any()
    output[first_column] = timestamps.dt.strftime("%Y-%m-%dT%H:%M:%S" if has_intraday_time else "%Y-%m-%d")
...
def fundamentals(request: FundamentalsRequest) -> dict[str, Any]:
    try:
